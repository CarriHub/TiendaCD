...
public class Registro extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        for (int i = 0; i < usuarios.size() && !encontrado; i++) {
            Usuario comprobar = usuarios.get(i);
            if (comprobar.getEmail().equalsIgnoreCase(email)) {
                error = "Este email ya esta registrado";
                encontrado = true;
            }
        }

        Usuario usu = new Usuario(nombre, apellido, email, Encriptador.encrypt(contrasena), direccion, null, null, null);
