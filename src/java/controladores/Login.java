    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (email != null && !email.trim().isEmpty() && contrasena != null && !contrasena.trim().isEmpty()) {
            ...
            for (int i = 0; i < usuarios.size() && !encontrado; i++) {
                Usuario usuarioComprobar = usuarios.get(i);
                if (usuarioComprobar.getEmail().equalsIgnoreCase(email)) {
                    encontrado = true;
                    if (usuarioComprobar.getContraseña().equals(Encriptador.encrypt(contrasena))) {
                        usu = usuarioComprobar;
                    }
                }
            }
            if (usu == null) {
